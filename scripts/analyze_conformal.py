...
Produces:
  - outputs/conformal_calibration.json   (per-alpha calibration details)
  - outputs/conformal_thresholds.json    (thresholds at each alpha)
  - outputs/conformal_results.json       (full test-set results: coverage, set sizes, safety cases)
...
Usage:
  python scripts/analyze_conformal.py \
      ...
      --alphas 0.05,0.10 \
      [--val-npz outputs/val_calib.npz]  # skip recomputation if it exists
"""
...
def main():
    ...
    parser.add_argument('--val-npz', default='outputs/val_calib.npz',
                        help='If exists, skip recomputing val probs')
    parser.add_argument('--hyper-weight', type=float, default=None,
                        help='HyperColumn weight (auto-detect from outputs/hypercolumn_weight.txt if omitted)')
